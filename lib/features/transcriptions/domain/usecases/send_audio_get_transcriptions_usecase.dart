...
@lazySingleton
class SendAudioGetTranscriptsUseCase
    implements UseCaseTwoResults<Transcripts, String> {
  SendAudioGetTranscriptsUseCase(this._transcriptsRepository);
  ...
  Future<Either<String, Transcripts>> call({required String params}) async {
    final eitherResult =
        await _transcriptsRepository.sendAudioGetTranscript(filePath: params);

    return eitherResult.fold(
      (exception) =>
          Left(_mapExceptionToString(exception)),
